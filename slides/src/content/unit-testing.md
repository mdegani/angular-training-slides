...
- We have several broken tests that we'll simply ignore for now. Use `fdescribe` to tell Jasmine to "**f**ocus" on this test so we don't see the other failing tests.
- Here we use the `toEqual` Jasmine matcher to perform a deep equality check. The `toBe` matcher, which performs a strict equality check (`===`), would also work here.
...
- The `componentInstance` property of the fixture is an instance of `AppComponent`. Here, we assign it to the `comp` variable to be used later in our tests.
...
- We could provide mocks for these components, or do a shallow render by adding the `CUSTOM_ELEMENTS_SCHEMA` to the `schemas` property.
...
- We import the real module that declares the component and provides the service it depends on
- We get the instance of `ToDoService` as we did before and give it an array of fake values.
...
## Using `fakeAsync` with a Jasmine Spy

- This test tests a 300 ms debouncing of changes *without* actually waiting for real time to pass
- The `spyOn` function allows us to tell Jasmine we are interested in tracking the `queryChanged` method.
- The matchers used below track when and if the method is called.

```ts
it('should debounce change to search query for 300 ms', fakeAsync(() => {

  // assume comp is an instance of the component under test
...
- Rather than providing a full mock for Angular's Http service, a light mock can be used.
- Note the use of `as` to assert the type of our mock.
- A Jasmine spy provides the mock implementation of the get method by forcing the `get` method to return the object we specific -- in this case an Observable.
...
<!-- .slide: id="unit-testing-quiz-1" -->

## Quiz

How can a pipe be unit tested?

1. You must use `TestBed` because pipes only work in the Angular runtime.

2. Pipes can't be unit tested in Angular, so always perform integration tests to make sure they work.

3. Use an isolated test and test the pipe's `transform` method.

4. Pipes should not contain logic, so they don't require tests.
...
1: Incorrect. For unit testing, `TestBed` is not required.

2: Incorrect. You test how pipes work in your templates by performing an integration test, but this is not the only way to test them.

3: Correct. Pipes are usually simple to unit test.

4: Incorrect. A pipe will contain a transformation and testing might be a good idea.

---

<!-- .slide: id="unit-testing-quiz-2" -->

## Quiz

When should you consider using TestBed?

1. Always -- Angular classes always have special dependencies and don't work without the Angular runtime.

2. When you want to perform an integration test.

3. Whenever you want to unit test components -- they require TestBed because they have templates.

4. When the element you're testing injects dependencies and you want Angular to provide them.
...
1: Incorrect. Angular uses regular ES2015/TypeScript classes and they can often be unit tested without Angular.

2: Correct. Testing the integration of services and components, for example, is a good reason to use TestBed.

3: Incorrect. It's true that components have templates, but if you only need to unit test a method, you probably don't need TestBed.

4: Correct. If you want to use Angular's DI framework, you should use TestBed.

---

<!-- .slide: id="unit-testing-quiz-3" -->

## Quiz

What does the `async` function do?

1. It runs your test in a zone and ensures that your tests run, even if they are in a callback.

2. It makes asynchronous code synchronous so that it is easier to test.

3. It let's you control the passage of time using the `tick` function.

4. It brings automatic change detection to your tests.
...
1: Correct. An `expect` placed in a callback will run when the test is wrapped in `async`.

2: Incorrect. Asynchronous things still asynchronously.

3: Incorrect. `tick` is only available inside the `fakeAsync` function.

4: Incorrect. While zones help with change detection, automatic change detection is enabled separately. 
